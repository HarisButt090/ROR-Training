...
puts "Output for Input [a,b,c] should be 'ac'"
puts first_and_last(["a","b","c"])
puts 'first_and_last(["bob", "tom", "rob"])  => "bobrob"'
...
def product_of_even_indices(array)
  product = 1
  # Multiply only elements at even indexes (0, 2, 4)
  [0, 2, 4].each do |i|
    product *= array[i]
  end
  return product
...
puts first_letter_of_last_string(["nonesense"])

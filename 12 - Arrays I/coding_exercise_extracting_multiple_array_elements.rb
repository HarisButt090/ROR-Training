...
def split_in_two(array)
  ...
  mid = (array.length / 2.0).ceil # ceil round off to the next big integer so we dont have to check even and odd 
  
  
  
  first_array = array[0,mid]  
  second_array = array[mid.. -1] # the .. indicates from mid to there # .. is called range opeartor
...
puts '  split_in_two(["A", "B"])                => [["A"], ["B"]]'
p split_in_two(["A","B"])

puts # to add line break

puts '  split_in_two(["A", "B", "C", "D"])      => [["A", "B"], ["C", "D"]]'
p split_in_two(["A","B","C","D"])

puts # to add line break

puts '  split_in_two(["A", "B", "C"])           => [["A", "B"], ["C"]]'
p split_in_two(["A","B","C"])

puts # to add line break

puts '  split_in_two(["A", "B", "C", "D", "E"]) => [["A", "B", "C"], ["D", "E"]]'
